...
Testing/launch tooling (2026-08-13): `make smoke` (curl E2E, 13 checks), `make app-up/app-down/app-logs` (full app in docker, `docker-compose.dev.yml`, test login wife/devpass), `make dev-ref` (small dev reference db), `make e2e` (headless Chromium via playwright container, screenshot to tests/e2e/).

Client PWA (2026-08-13): login screen, reference-db loader (manifest → sha256-verified, `no-store` fetch + `no-cache`/`no-store` server headers — HTTP cache can't serve stale ref bytes), villager list + search (417 villagers, avatar blobs from `images` table), villager detail "about" page. Routing: **sv-router 0.18.1** (vetted: npm provenance/SLSA, 1 dep `esm-env` already in tree, no lifecycle scripts, passes pnpm guardrails; TanStack has no official Svelte support, svelte-routing is dead). Deep links `/villager/:name` + browser back/forward work; reference db load is app-level (App.svelte) so deep links render without visiting the list. Write-db spec drafted in plan.md (favorite/on-island/gifts locked; notes + wishlist proposed). **Gift matcher built (2026-08-13)**: `build_db.py` now extracts clothing `Style 1`/`Style 2` into `items.style` (was reading a nonexistent 'Style' column — 5,888 items now have styles) + `label_themes` column; `client/src/lib/gifts.ts` ranks giftable items by color (furniture) / color+style (clothing, "★ Perfect match" at score 2); VillagerDetail shows "Gift ideas" + "Their house" strip. Validated against archived Nookea pages (color-driven recommendations; Chow = Black/White clothing = his favorite colors). `make client-check/build` green; `make e2e` passes (login → db → list → search → click URL → goBack → deep links → gift ideas). Next: implement progress.db client (create/ensureSchema/write/save), then gift log, favorites, sync, service worker.
