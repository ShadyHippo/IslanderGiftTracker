...
Client PWA (2026-08-13): login screen, reference-db loader (manifest → sha256-verified, `no-store` fetch + `no-cache`/`no-store` server headers — HTTP cache can't serve stale ref bytes), villager list + search (417 villagers, avatar blobs from `images` table), villager detail "about" page. Routing: **sv-router 0.18.1** (vetted: npm provenance/SLSA, 1 dep `esm-env` already in tree, no lifecycle scripts, passes pnpm guardrails; TanStack has no official Svelte support, svelte-routing is dead). Deep links `/villager/:name` + browser back/forward work; reference db load is app-level (App.svelte) so deep links render without visiting the list. Write-db spec drafted in plan.md (favorite/on-island/gifts locked; notes + wishlist proposed). **Furniture type catalog (2026-08-14)**: two-level category tree (e.g. `Kitchen/Appliance/Fridge`, `Decoration/Instrument/Guitar`) assigned to all 2,028 furniture items — `scripts/furniture_types.py` (rules + curated overrides, single source of truth), written to `items.type_path` by every build (reproducible; the catalog is code + `res/furniture_types.tsv` review artifact). 100% coverage, 0 unclassified. `build_db.py` also builds atomically (`.tmp` + `os.replace`, never blanks the served gz) and caches images in git-ignored `images/raw` + `images/thumb` (wiki results cached too). Next: hierarchical filter pills (furniture/clothing types + buyable, OR for types / AND for buyable).
